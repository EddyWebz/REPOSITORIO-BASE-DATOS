...
function imprimirReporte() {
    ...
    if (reportesContainer.innerHTML.trim() === "") {
        alert("Los datos aún no se han cargado. Por favor, espera un momento e inténtalo de nuevo.");
        return;
    }
    
    // Forzar un re-renderizando de la vista antes de imprimir
    const originalContents = document.body.innerHTML;
    const printContents = reportesContainer.innerHTML;

    // Forzar la actualización del contenido antes de imprimir
    document.body.innerHTML = printContents;

    // Esperar un pequeño intervalo para asegurarse de que el contenido esté actualizado
    setTimeout(() => {
        window.print();
