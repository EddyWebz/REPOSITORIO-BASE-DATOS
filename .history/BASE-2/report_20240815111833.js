...
function imprimirReporte() {
    const printContents = document.getElementById('reportes').innerHTML;
    const printWindow = window.open('', '', 'height=600,width=800');

    printWindow.document.write('<html><head><title>Imprimir Reporte</title>');
    printWindow.document.write('<link rel="stylesheet" href="notif.css">');  // Asegúrate de cargar el CSS correcto
    printWindow.document.write('</head><body>');
    printWindow.document.write(printContents);
    printWindow.document.write('</body></html>');

    printWindow.document.close();  // Necesario para que funcione en algunos navegadores

    printWindow.onload = function() {
        printWindow.focus();  // Asegúrate de que la ventana de impresión tenga el foco
        printWindow.print();  // Inicia el proceso de impresión
        printWindow.close();  // Cierra la ventana de impresión después de que se complete
    };
}
