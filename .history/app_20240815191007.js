...
const notasRoutes = require('./notas');  // Añadir la ruta de notas.js
...
// Proteger la ruta a cuerpo.html (que está fuera de la carpeta pública)
app.get('node_modules/html2pdf.js/dist', isAuthenticated, (req, res) => {
    res.sendFile(path.join(__dirname, 'node_modules/html2pdf.js/dist')); // Envía el archivo cuerpo.html si está autenticado
});
...
// Ruta para servir la página de notificaciones y reportes
app.get('/privado/report.html', isAuthenticated, (req, res) => {
    res.sendFile(path.join(__dirname, 'privado', 'report.html'));
});
// Servir toda la carpeta 'html2pdf.js' desde 'node_modules'
app.use('/html2pdf.bundle.min.js', express.static(path.join(__dirname, 'node_modules/html2pdf.js/dist')));
        ...
        if (results.length > 0) {
            ...
            bcrypt.compare(password, results[0].password, (err, isMatch) => {
                ...
                if (isMatch) {
                    ...
                    console.log('User ID stored in session:', req.session.userId); // Depuración
...
app.post('/register', (req, res) => {
    ...
    bcrypt.hash(password, 10, (err, hashedPassword) => {
        ...
        connection.query(sql, [nombre, email, hashedPassword], (err, result) => {
            ...
            // El user_id es el ID del registro recién creado
            const userId = result.insertId;
            req.session.userId = userId;  // Guardar el user_id en la sesión
            res.status(201).json({ success: true, userId });  // Devolver el user_id al frontend
        });
    });
});
